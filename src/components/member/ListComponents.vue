<template>
    ...
    <div class="bg-slate-950 p-2 pt-0 flex justify-center">
      <router-link to="/members/create" class="w-full h-10 rounded-ss-full p-2 mt-0 border-2 text-center bg-slate-900 hover:bg-slate-700 text-white font-bold">Agregar</router-link>
    </div>
    ...
    <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
      ...
      <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
            ...
            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
              ...
              <div class="mt-2">
                <p class="text-sm text-gray-500 font-bold">
                  ¿ Esta seguro que desea eliminar al Miembro {{modalMember.name}} {{modalMember.lastname}} con la cédula {{modalMember.card}} ?
                </p>
              </div>
            </div>
          </div>
        ...
        <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse">
          ...
          <button @click="confirm(modalMember.id)" type="button" class="mt-3 w-full inline-flex justify-center rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-white text-base font-medium text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:mt-0 sm:ml-3 sm:w-auto sm:text-sm">
            Confirmar
          </button>
        </div>
      </div>
...
const openModal = (id) => {
  modalMember.value = store.getMember(id)
